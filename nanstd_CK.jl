...
function nanstd_CK(data::Matrix, dim::Int)
...
function nanstd_CK(data::Array{T, 3} where T, dim::Int)
    # compute std of a matrix along certain dimension while ignoring NAN values
    # by Caesar

    if dim == 1
        stdv = map(i -> try nanstd_CK(data[:, :, i], 1) catch; NaN end, 1:size(data, 3))
        stdv = hcat(stdv...)
    elseif dim == 2
