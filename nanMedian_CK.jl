function nanMedian_CK(data::Array)
...
function nanMedian_CK(data::Matrix, dim::Int)
    # compute median of data while ignoring NAN values
    # by Caesar
...
function nanMedian_CK(data::Array{T, 3} where T, dim::Int)
    # compute median of a matrix along certain dimension while ignoring NAN values
    # by Caesar

    if dim == 1
        mid = map(i -> try nanMedian_CK(data[:, :, i], 1) catch; NaN end, 1:size(data, 3))
        mid = hcat(mid...)
    elseif dim == 2
