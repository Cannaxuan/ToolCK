...
function nanSum_CK(data::Matrix, dim::Int)
    # compute sum of data while ignoring NAN values
    # by Caesar
...
function nanSum_CK(data::Array{T, 3} where T, dim::Int)
    # compute sum of a matrix along certain dimension while ignoring NAN values
    # by Caesar

    if dim == 1
        su = map(i -> try nanSum_CK(data[:, :, i], 1) catch; NaN end, 1:size(data, 3))
        su = hcat(su...)
    elseif dim == 2
